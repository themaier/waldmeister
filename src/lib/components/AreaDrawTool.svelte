<script lang="ts">
  ...
  $effect(() => {
    ...
    // Lock document scroll while drawing — see RouteDrawTool.svelte for the
    // reasoning. Inline touch-action: none on the canvas isn't reliable on
    // mobile when the host page is scrollable; pinning scrollY via
    // `position: fixed` on body is the only lock that survives.
