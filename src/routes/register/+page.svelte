<script lang="ts">
  ...
  const features = [
    'Unbegrenzt viele Waldstücke',
    'GPS-Erfassung mit Foto',
    'Auftragslinks mit Ablauffrist',
    'Offline-fähige PWA'
  ];
</script>
...
<div class="lodge-bg grid grid-cols-1 lg:grid-cols-[1.05fr_1fr] min-h-dvh">
  <aside class="hidden lg:flex flex-col gap-8 p-10 relative z-[1] text-earth">
    ...
    <div class="mt-auto flex flex-col gap-3">
      ...
      >
        Zeichne jeden<br /><em class="italic text-leaf" style="font-variation-settings: 'opsz' 144, 'SOFT' 100, 'WONK' 1;">deiner Bäume.</em>
      </h2>
      <p class="max-w-[38ch] text-base leading-relaxed" style="color: color-mix(in srgb, var(--color-earth) 78%, transparent);">
        Ein Konto genügt, um Waldstücke anzulegen, Bäume zu erfassen und Aufträge zu teilen — von der Flurstücksgrenze bis zum einzelnen Stamm.
      </p>
    </div>

    <ul class="list-none m-0 p-0 flex flex-col gap-2">
      {#each features as f}
        <li class="flex items-center gap-3 text-[0.9375rem]" style="color: color-mix(in srgb, var(--color-earth) 82%, transparent);">
          <span
            class="w-1.5 h-1.5 rounded-full bg-leaf"
            style="box-shadow: 0 0 0 4px color-mix(in srgb, var(--color-leaf) 18%, transparent);"
          ></span>
          {f}
        </li>
      {/each}
    ...
    <p class="font-serif italic text-[0.9375rem]" style="color: color-mix(in srgb, var(--color-earth) 55%, transparent);">
      „Vom Sämling zum Schlagreifen — alles auf einer Karte.“
    </p>
  </aside>
  ...
  <main class="grid place-items-center p-5 bg-earth relative z-[1]">
    <div class="grain relative w-full max-w-[26rem] bg-surface border rounded-box shadow-canopy px-6 pt-8 pb-5 flex flex-col gap-5 animate-rise">
      ...
      <div class="text-center flex flex-col gap-1 mt-3">
        ...
        >
          Deinen Wald digitalisieren.
        </h1>
        <p class="text-content-muted text-sm">Ein Konto genügt. Keine Kreditkarte nötig.</p>
      </div>

      <form onsubmit={submit} class="flex flex-col gap-4">
        <label class="flex flex-col gap-2">
          <span class="text-[0.8125rem] font-semibold text-content">Name</span>
          <input
            type="text"
        ...
        <label class="flex flex-col gap-2">
          <span class="flex justify-between items-baseline text-[0.8125rem] font-semibold text-content">
            Passwort
            <span class="text-xs text-content-muted font-normal">mind. 8 Zeichen</span>
          </span>
          <input
            type="password"
            class="w-full px-4 py-3 min-h-[48px] rounded-btn border bg-earth text-[0.9375rem] text-ink focus:outline-none focus:border-pine focus:shadow-ring-focus transition"
        ...
        <button
          ...
          disabled={submitting}
        >
          <span>{submitting ? 'Konto erstellen …' : 'Konto erstellen'}</span>
      ...
      <div class="flex flex-col gap-3 text-center text-content-muted text-[0.8125rem]">
        ...
        <p>
          Schon ein Konto? <a href="/login" class="text-pine font-semibold no-underline hover:underline">Jetzt anmelden</a>
        </p>
      </div>
    </div>
  </main>
